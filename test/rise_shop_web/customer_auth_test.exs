defmodule RiseShopWeb.CustomerAuthTest do
  use RiseShopWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias RiseShop.Store
  alias RiseShopWeb.CustomerAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RiseShopWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_customer/3" do
    test "stores the customer token in the session", %{conn: conn, customer: customer} do
      ...
      assert Store.get_customer_by_session_token(token)
    end
  ...
  describe "logout_customer/1" do
    test "erases session and cookies", %{conn: conn, customer: customer} do
      customer_token = Store.generate_customer_session_token(customer)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Store.get_customer_by_session_token(customer_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "customers_sessions:abcdef-token"
      RiseShopWeb.Endpoint.subscribe(live_socket_id)
