defmodule RiseShop.StoreTest do
  ...
  describe "get_customer_by_email_and_password/2" do
    ...
    test "returns the customer if the email and password are valid" do
      ...
      assert %Customer{id: ^id} =
               Store.get_customer_by_email_and_password(customer.email, valid_customer_password())
    end
  end
  ...
  describe "get_customer!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Store.get_customer!(-1)
      end
    end

    test "returns the customer with the given id" do
      %{id: id} = customer = customer_fixture()
      assert %Customer{id: ^id} = Store.get_customer!(customer.id)
    end
  end

  describe "register_customer/1" do
    test "requires email and password to be set" do
      {:error, changeset} = Store.register_customer(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = Store.register_customer(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = Store.register_customer(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = Store.register_customer(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = Store.register_customer(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers customers with a hashed password" do
      email = unique_customer_email()
      {:ok, customer} = Store.register_customer(valid_customer_attributes(email: email))
  ...
  describe "change_customer_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = Store.change_customer_registration(%Customer{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Store.change_customer_registration(
          %Customer{},
  ...
  describe "change_customer_email/2" do
    test "returns a customer changeset" do
      assert %Ecto.Changeset{} = changeset = Store.change_customer_email(%Customer{})
  ...
  describe "apply_customer_email/3" do
    ...
    test "requires email to change", %{customer: customer} do
      {:error, changeset} = Store.apply_customer_email(customer, valid_customer_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{customer: customer} do
      {:error, changeset} =
        Store.apply_customer_email(customer, valid_customer_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{customer: customer} do
      ...
      {:error, changeset} =
        Store.apply_customer_email(customer, valid_customer_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{customer: customer} do
      ...
      {:error, changeset} = Store.apply_customer_email(customer, password, %{email: email})
    ...
    test "validates current password", %{customer: customer} do
      {:error, changeset} =
        Store.apply_customer_email(customer, "invalid", %{email: unique_customer_email()})
    ...
    test "applies the email without persisting it", %{customer: customer} do
      email = unique_customer_email()
      {:ok, customer} = Store.apply_customer_email(customer, valid_customer_password(), %{email: email})
      assert customer.email == email
      assert Store.get_customer!(customer.id).email != email
    end
  end
    ...
    test "sends token through notification", %{customer: customer} do
      token =
        extract_customer_token(fn url ->
          Store.deliver_customer_update_email_instructions(customer, "current@example.com", url)
        end)
  ...
  describe "update_customer_email/2" do
    setup do
      ...
      token =
        extract_customer_token(fn url ->
          Store.deliver_customer_update_email_instructions(%{customer | email: email}, customer.email, url)
        end)
    ...
    test "updates the email with a valid token", %{customer: customer, token: token, email: email} do
      assert Store.update_customer_email(customer, token) == :ok
    ...
    test "does not update email with invalid token", %{customer: customer} do
      assert Store.update_customer_email(customer, "oops") == :error
    ...
    test "does not update email if customer email changed", %{customer: customer, token: token} do
      assert Store.update_customer_email(%{customer | email: "current@example.com"}, token) == :error
      assert Repo.get!(Customer, customer.id).email == customer.email
      assert Repo.get_by(CustomerToken, customer_id: customer.id)
    end
    ...
    test "does not update email if token expired", %{customer: customer, token: token} do
      ...
      assert Store.update_customer_email(customer, token) == :error
      assert Repo.get!(Customer, customer.id).email == customer.email
      assert Repo.get_by(CustomerToken, customer_id: customer.id)
    end
  end
  ...
  describe "change_customer_password/2" do
    test "returns a customer changeset" do
      assert %Ecto.Changeset{} = changeset = Store.change_customer_password(%Customer{})
      assert changeset.required == [:password]
    end

    test "allows fields to be set" do
      changeset =
        Store.change_customer_password(%Customer{}, %{
  ...
  describe "update_customer_password/3" do
    ...
    test "validates password", %{customer: customer} do
      {:error, changeset} =
        Store.update_customer_password(customer, valid_customer_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{customer: customer} do
      ...
      {:error, changeset} =
        Store.update_customer_password(customer, valid_customer_password(), %{password: too_long})
    ...
    test "validates current password", %{customer: customer} do
      {:error, changeset} =
        Store.update_customer_password(customer, "invalid", %{password: valid_customer_password()})
    ...
    test "updates the password", %{customer: customer} do
      ...
      assert is_nil(customer.password)
      assert Store.get_customer_by_email_and_password(customer.email, "new valid password")
    end

    test "deletes all tokens for the given customer", %{customer: customer} do
      _ = Store.generate_customer_session_token(customer)
  ...
  describe "generate_customer_session_token/1" do
    ...
    test "generates a token", %{customer: customer} do
      token = Store.generate_customer_session_token(customer)
  ...
  describe "get_customer_by_session_token/1" do
    ...
    test "returns customer by token", %{customer: customer, token: token} do
      assert session_customer = Store.get_customer_by_session_token(token)
      assert session_customer.id == customer.id
    end

    test "does not return customer for invalid token" do
      refute Store.get_customer_by_session_token("oops")
    end

    test "does not return customer for expired token", %{token: token} do
      {1, nil} = Repo.update_all(CustomerToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute Store.get_customer_by_session_token(token)
    end
  end
  ...
  describe "delete_customer_session_token/1" do
    test "deletes the token" do
      customer = customer_fixture()
      token = Store.generate_customer_session_token(customer)
      assert Store.delete_customer_session_token(token) == :ok
      refute Store.get_customer_by_session_token(token)
    end
  end
  ...
  describe "confirm_customer/1" do
    ...
    test "confirms the email with a valid token", %{customer: customer, token: token} do
      assert {:ok, confirmed_customer} = Store.confirm_customer(token)
    ...
    test "does not confirm with invalid token", %{customer: customer} do
      assert Store.confirm_customer("oops") == :error
    ...
    test "does not confirm email if token expired", %{customer: customer, token: token} do
      {1, nil} = Repo.update_all(CustomerToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert Store.confirm_customer(token) == :error
      refute Repo.get!(Customer, customer.id).confirmed_at
      assert Repo.get_by(CustomerToken, customer_id: customer.id)
    end
  end
  ...
  describe "deliver_customer_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{customer: customer} do
      token =
        extract_customer_token(fn url ->
          Store.deliver_customer_reset_password_instructions(customer, url)
        end)
  ...
  describe "reset_customer_password/2" do
    ...
    test "validates password", %{customer: customer} do
      {:error, changeset} =
        Store.reset_customer_password(customer, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{customer: customer} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = Store.reset_customer_password(customer, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{customer: customer} do
      {:ok, updated_customer} = Store.reset_customer_password(customer, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given customer", %{customer: customer} do
      _ = Store.generate_customer_session_token(customer)
      {:ok, _} = Store.reset_customer_password(customer, %{password: "new valid password"})
      refute Repo.get_by(CustomerToken, customer_id: customer.id)
    end
  end
