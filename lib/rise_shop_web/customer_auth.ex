defmodule RiseShopWeb.CustomerAuth do
  use RiseShopWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_rise_shop_web_customer_remember_me"
  ...
  def log_out_customer(conn) do
    ...
    customer_token && Store.delete_customer_session_token(customer_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      RiseShopWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_customer(conn, _opts) do
    ...
    customer = customer_token && Store.get_customer_by_session_token(customer_token)
  ...
  the current_customer:
      ...
      defmodule RiseShopWeb.PageLive do
        use RiseShopWeb, :live_view

        on_mount {RiseShopWeb.CustomerAuth, :mount_current_customer}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{RiseShopWeb.CustomerAuth, :ensure_authenticated}] do
  ...
  defp mount_current_customer(socket, session) do
    Phoenix.Component.assign_new(socket, :current_customer, fn ->
      if customer_token = session["customer_token"] do
        Store.get_customer_by_session_token(customer_token)
      end
    end)
  end
